...
sub process_cpp {
    ...
    {
        my $data = join("", @data);
        @data = filter_program($data, "uncrustify", "-q", "-c", "scripts/uncrustify.cfg", "-l", "CPP");
...
(-e "include/stx/btree.h")
    or die("Please run this script in the stx-btree source base directory.");
...
foreach my $file (@filelist)
{
    ...
    elsif ($file =~ m!^wxbtreedemo/.*_wxg\.!) {
    }
    elsif ($file =~ /\.(h|c|cc|cpp|hpp|h.in)$/ or
           $file =~ /^include\/stx/) {
        process_cpp($file);
    }
    ...
    elsif ($file =~ m!\.(o|xcf|ico|wxg|xpm|rc|xml|gnuplot|in|am|txt|trt|pdf|awk)$!) {
    }
    elsif ($file =~ m!(^|\/)(README|AUTHORS|Makefile)$!) {
    }
    elsif ($file =~ m!(^|\/)(\.deps/|commit-.*)!) {
    }
